 ...
 * - Description: This struct represents a `SwiftUI` view that can be used to display an edit-menu with customizable actions and callbacks for presentation and dismissal.
 *                It conforms to the `UIViewRepresentable` protocol to integrate with UIKit views.
 * - Note: This doesn't need to be public
 * - Fixme: ⚠️️ Add some sort of accessibility-id?
 */
struct EditMenu<Content: View>: UIViewRepresentable {
   /**
    * Content -  How the button should look like
    * - Description: This is the visual representation of the button that triggers the edit menu. It can be customized to fit the design requirements of the application.
    */
   let content: Content
   /**
    * Actions - Has the title, icon and callbacks for the menu
    * - Description: This is a closure that returns an array of UIAction objects. Each UIAction represents an action that can be performed from the edit menu. The UIAction includes a title, an optional image, and a handler that is called when the action is selected.
    */
   let actions: () -> [UIAction]
   /**
    * onPresent - Callback for swiftui
    * - Description: This callback is triggered when the edit menu is presented, allowing for additional actions or state updates in response to the menu's presentation.
    */
   let onPresent: EditMenuCallBack?
   /**
    * onDismiss - Callback for swiftui
    * - Description: This callback is triggered when the edit menu is dismissed, allowing for additional actions or state updates in response to the menu's dismissal.
    */
   let onDismiss: EditMenuCallBack?
   /**
    * UIViewType - The type of the UIView
    * - Description: Defines the type of UIView that will be used to represent the SwiftUI view in the UIKit environment.
    */
   typealias UIViewType = UIView
   /**
    * Creates and configures the UIView for the EditMenu.
    * - Description: This function is responsible for creating the UIView that will be used to present the edit menu. It sets up the necessary interactions and gesture recognizers to handle user input and display the menu accordingly.
    * - Parameters:
    ...
    * - Description: This method is called whenever the SwiftUI state has changes that need to be reflected in the UIKit representation of the view. It ensures that the UIView remains in sync with SwiftUI's state and lifecycle events
    * - Parameters:
    ...
    * - Description: The `makeCoordinator` method creates a Coordinator object that serves as the communication hub between the SwiftUI view and the UIKit-based edit menu interaction. The Coordinator is responsible for managing the lifecycle of the UIEditMenuInteraction, handling user gestures, and presenting the edit menu.
    */
